...
{
    // Left
    uint8_t ll = proxSensors.countsLeftWithLeftLeds();
    uint8_t rl = proxSensors.countsRightWithLeftLeds();
    uint8_t left = max(ll, rl);

    // Right
    uint8_t rr = proxSensors.countsRightWithRightLeds();
    uint8_t lr = proxSensors.countsLeftWithRightLeds();
    uint8_t right = max(rr, lr);
    
    // Center
    uint8_t fl = proxSensors.countsFrontWithLeftLeds();
    uint8_t fr = proxSensors.countsFrontWithRightLeds();
    uint8_t center = max(fl, fr);
